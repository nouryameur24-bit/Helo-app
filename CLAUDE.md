...
36. **Audit-refacto itératif module par module (02/07/2026 nuit)** : passe qualité (pilier « ligne par ligne / DRY / perf », le moins couvert par les audits 1-4). Un module = un commit, tsc+250 tests verts à chaque.
   ...
   - **Reste à passer** (ordre) : lib/hooks, backend (refacto only). Amélioration proposée non faite sur module 1 : extraire 3 blocs IIFE du verdict (`AnalysisSourceCard`, `ContextualQuoteRow`, `CommunityBadge`) en composants mémoïsés. Module 3 : handoff base64→AsyncStorage (moat, à valider).
