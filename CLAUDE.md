...
37. **Push qualité S+ (02/07/2026 nuit)** — 3 dettes attaquées :
   ...
   - **Dark mode** (commit `bc53041`) : toggle « Lune de minuit » (PreferencesSection) **retiré** — c'était une promesse cassée (0 écran ne consomme `useColors()`, 185 fichiers importent `Colors` en statique, tous les `StyleSheet.create` capturent les couleurs au load). Vrai dark mode = réécriture structurelle (185 fichiers), hors scope. Infra `useAppTheme` + `ColorsDark` gardée **dormante** pour une future migration dédiée.
