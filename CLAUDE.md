...
36. **Audit-refacto itératif module par module (02/07/2026 nuit)** : passe qualité (pilier « ligne par ligne / DRY / perf », le moins couvert par les audits 1-4). Un module = un commit, tsc+250 tests verts à chaque.
   ...
   - **Module 5 `shelf.tsx` + `components/shelf/*`** (commit `9ac0d42`) : 🔴 **BUG MAJEUR** — `MonPlacardView` ne chargeait JAMAIS le placard de la maman : l'effet gérait le partenaire (Supabase realtime) mais la branche `else` (persona principale) faisait `setProducts([])`, et `useProfile` n'y destructurait même pas `userId` (commentaire « vient du hook parent » = stale, refacto à moitié fait) → « Mon Placard » **toujours vide** pour la maman malgré les ajouts. Fix : réutilise `useShelfData` (hook éprouvé du home) pour le mode maman ; partenaire garde son realtime. + 2 bugs latents : `handleRemove` persiste maintenant la suppression (avant perdue au remount), `handleRefresh` recharge vraiment (avant setTimeout décoratif). Vérif runtime recommandée.
   - **Module 6 écrans secondaires + code mort** (commit `fb35d12`) : écrans feature-flaggés bien gatés (ComingSoonScreen), écrans de calcul (nutrition/home-score/weekly-brief/timeline) chargent correctement (pas de bug type module 5), aucun stub. Nettoyage : **75 déclarations mortes** que tsc ne voyait pas (`noUnusedLocals` off) — 57 imports, 3 consts data mortes (HISTORY_DATA/MOOD_EMOJI/COURSE_LABELS), vars/params. 🔒 **`noUnusedLocals` + `noUnusedParameters` activés dans `tsconfig.json`** → ⚠️ **désormais tout import/var mort FAIT ÉCHOUER la CI** (le typecheck mobile est bloquant). Nouveau code : ne laisse aucun import inutilisé.
   - **Module 7 `lib/*` + `hooks/*`** (commit `3745256`) : libs saines (glowscore div0/clamp OK, trimester borne T2/T3 conservatrice voulue, matcher/preference déjà testés). 🔴 **BUG** : `syncOfflineScans` (lib/offline.ts) insérait dans une table **`scans` INEXISTANTE** (seul `scan_history` existe) → sync hors-ligne KO à 100%, file jamais vidée (croissance sans fin). Fix : repointe vers `scan_history` RLS-valide (user_id=session, product_id résolu par barcode, in_shelf=false) → scans hors-ligne réapparaissent dans Historique + stats profil. ⚠️ **non vérifié runtime** (mode offline = Premium, besoin device). Dédup `parseIngredients`+`matchSafeOverride` mobile↔backend : **laissée** (synchro + gardée par tests ; extraction package monorepo jugée trop risquée avant beta) — drift à surveiller manuellement.
...
37. **Push qualité S+ (02/07/2026 nuit)** — 3 dettes attaquées :
   - **Accessibilité** (commits `a73ea63`, `713212c`, `9ecfac2`) : axe le plus faible (~54%) élevé. ⚠️ **`Button` + `IconButton` (`components/ui/`) portent désormais `accessibilityRole="button"` + `accessibilityState` PAR DÉFAUT** → tout usage annoncé comme bouton par VoiceOver (55 usages d'un coup). `IconButton` icon-only → **passer un `accessibilityLabel`** obligatoire. Flux principaux 100% + 34 boutons icon-only sur 28 écrans. `accessibilityLabel` 170→219. Convention : tout nouveau bouton icon-only DOIT avoir un `accessibilityLabel`. Écrans feature-flag-off non traités (volontaire).
   - **Découpe** (commits `447fe19`, `c5764d4`, `ba167cd`, `3181810`, `514c5ab`) : (1) `verdict/[scanId].tsx` **1114→976 L** — 3 IIFE inline extraits en composants mémoïsés `components/verdict/VerdictHeroCards.tsx` (AnalysisSourceCard/ContextualQuoteRow/CommunityContributionBadge). (2) **Extraction des blocs `StyleSheet.create` trailing en fichiers `*Styles.ts` frères** (convention établie verdictStyles/homeStyles) sur 12 écrans : chat 811→469, restaurant-results 686→542, submit-product 566→430, community 618→371, scan 809→627, guide/methodology/history/prescription-results/basket-results/onboarding-profile, timeline 763→512. ⚠️ **Nouveau pattern : un écran avec un gros StyleSheet trailing → le sortir en `<screen>Styles.ts`** (`export const styles`), source importe `{ styles }`. `noUnusedLocals` force le nettoyage des imports theme devenus morts. Fichiers >500 L : 19→15 (les 15 restants sont soit logique légitime — productLookup/useScan/api/ocr-review — soit multi-block/consts partagées à faible ROI).
   - **Dark mode** (commit `bc53041` retrait toggle, `bb53b94` socle) : d'abord toggle « Lune de minuit » **retiré** (promesse cassée). Puis **Phase 1 du vrai dark mode** : nouveau `hooks/ThemeProvider.tsx` = **Context de thème PARTAGÉ** au root (`app/_layout.tsx`) qui corrige le défaut BLOQUANT de l'ancien `useAppTheme` (état par-hook non partagé → toggler ne notifiait aucun écran, dark mode impossible même migré). `useAppTheme.ts` ré-exporte pour compat. `useColors()`/`useAppTheme()` lisent le context. **Zéro écran migré → light intact, zéro régression.** ⚠️ **RÉALITÉ TOUT-OU-RIEN découverte** : `ThemedText` (primitif texte, 200+ fichiers) résout ses couleurs en statique → migrer 1 écran seul rend TOUS les autres illisibles en dark (texte clair migré / fond clair statique). Il n'y a PAS de tranche partielle sûre : le rollout visuel = migrer ThemedText + les ~185 fichiers ensemble (`StyleSheet.create` statique → factory `makeStyles(c)` dynamique via `useColors`), **à valider sur device** (le dark mode se vérifie à l'œil, pas au tsc). Projet dédié multi-session. Le socle (Phase 1) est prêt et débloque cette migration.
