...
23. **Hēlo Points Phase 1 MVP** (commit `f68c4ec`, task #107) : Système de récompenses gamifié pour incentiver Ghost Capture et combler gap couverture vs Yuka. Schema : `user_points` (balance/tier/streak), `point_transactions` (ledger RLS self-read), `rewards_catalog` (8 récompenses seedées), `point_redemptions`. RPCs atomiques `award_points` (cap 300/jour, 1 barcode = 1 award/reason) + `redeem_reward` (FOR UPDATE lock). Pricing : Photo INCI=25 (le gros), barcode=5, nom=5, face=8 (futur), marque=7 (futur), bonus complétude=+30 all-or-nothing. Récompenses Phase 1 (0€ coût) : themes/badges/+chat puis Premium 1 sem (300)/1 mois (800)/6 mois (2500)/1 an (5000 pts). Mobile : `lib/heloPoints.ts`, `components/points/PointsToast.tsx`, `components/profile/HeloPointsSection.tsx`, `app/points.tsx`. Wiring ocr-review: award fire-and-forget après `ghostCaptureSave` → +30-35 pts/contribution. **À finir Phase 1.5** : (a) PointsToast wiré dans ocr-review (toast visible), (b) fulfillment RPC redeem_reward côté api-server (Premium unlock via RevenueCat), (c) détection 1st-contributor + multiplier ×2, (d) bonus complétude quand step brand+face existeront. Phase 2 post-revenue : gift cards Amazon Incentives API.
