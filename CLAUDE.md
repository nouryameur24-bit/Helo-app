...
37. **Push qualité S+ (02/07/2026 nuit)** — 3 dettes attaquées :
   ...
   - **Découpe** (commit `447fe19`) : `verdict/[scanId].tsx` **1114→976 L** — 3 IIFE inline (carte provenance + citation + badge communauté) extraits en composants mémoïsés `components/verdict/VerdictHeroCards.tsx` (AnalysisSourceCard/ContextualQuoteRow/CommunityContributionBadge). Comportement identique. `alternatives.tsx` (1079 L) reste à découper (déjà partiellement décomposé en fonctions internes).
